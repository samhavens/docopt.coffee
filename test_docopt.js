 ...
 */
const print = function() { return console.log.apply(this, [].slice.call(arguments)); };


class WasNotRaisedError extends Error {}

const [setup, teardown, test, eq] = Array.from((function() {
    ...
    return [function(f) {
        results = [];
        console.log = function() { return buffer += ([].join.call(arguments, ' ')) + '\n'; };
        const pseudoso =
            {write() { return buffer += [].join.call(arguments, ' '); }};
    ...
    function(f) {
        ...
        for (result of Array.from(results)) {
            if (result[0] === 'F') {
                ...
                if (half > 0) {
                    [left, right] = Array.from([new Array(half + (half%1)).join('=') + ' ',
        ...
        cl(((() => {
            const result1 = [];
            for (result of Array.from(results)) {                 result1.push(result[0]);
            }
            return result1;
        })()).join(''));
    ...
    function(a, b) {
        ...
        const bs = b.toString();
        setsize++;
        if (as === bs) { return; } else { throw new Error(`${as} != ${bs}`); }
    }];
})());
...
test("parse_args", function() {
    const test_options = [new Option(null, '--all'), new Option('-b'), new Option('-W', null, 1)];
    eq(
...
-v  Be verbose.`;
    ...
    return eq(docopt(doc, {'argv': '-q arg'}), new Dict([['A', 'arg'], ['-v', false], ['-q', true]]));
});
...
test("commands", function() {
    ...
    return eq(docopt('Usage: prog a b', {'argv': 'a b'}), new Dict([['a', true], ['b', true]]));
});
    //with raises(DocoptExit):
    //    assert docopt('Usage: prog a b', 'b a')


test("parse_doc_options", function() {
...
test("printable_and_formal_usage", function() {
    const doc = `\
Usage: prog [-hv] ARG
       prog N M
...
prog is a program.`;
    ...
    return eq(printable_usage('uSaGe: prog ARG\n\t \t\n bla'), "uSaGe: prog ARG");
});


test("parse_args2", function() {
    const o = [new Option('-h'), new Option('-v', '--verbose'), new Option('-f', '--file', 1)];
    eq(
        parse_args('', o),
        []
    );
...
test("parse_pattern", function() {
    ...
    eq(parse_pattern('<add>', o), new Required([new Argument('<add>')]));
    return eq(parse_pattern('add', o), new Required([new Command('add')]));
});
...
test("option_match", function() {
    ...
    return eq(new Option('-a', null, false).match([new Option('-a', null, false)]), 
            [true, [], []]);
});
...
test("argument_match", function() {
    ...
    return eq(new Argument('N').match([new Argument(null, 9), new Argument(null, 0)]), [
...
test("command_match", function() {
    ...
    return eq(new Either([new Command('add', false), new Command('rm', false)]).match(
...
test("required_match", function() {
    ...
    eq(new Required([new Option('-a'), new Option('-b')]).match([new Option('-a')]), [
            false, [new Option('-a')], []]);
    return eq(new Optional([new Option('-a'), new Option('-b')]).match(
...
test("either_match", function() {
    ...
    return eq(new Either([new Argument('M'),
...
//                    (true, [new Option('-x')], [new Argument('N', 1), new Argument('N', 2)])
    return eq(new OneOrMore([new Optional([new Argument('N')])]).match([new Argument(null, 9)]), 
                    [true, [], [new Argument('N', 9)]]);
});
...
test("list_argument_match", function() {
    ...
    eq(new Required([new Argument('N'), new OneOrMore([new Argument('N')])]).fix().match(
            [new Argument(null, 1), new Argument(null, 2), new Argument(null, 3)]), 
                    [true, [], [new Argument('N', [1, 2, 3])]]);
    return eq(new Required([new Argument('N'), new Required([new Argument('N')])]).fix().match(
            [new Argument(null, 1), new Argument(null, 2)]), 
                    [true, [], [new Argument('N', [1, 2])]]);
});
...
test("options_without_description", function() {
    ...
    return eq(docopt('usage: git remote [-v | --verbose]', {argv: 'remote -v'}),
